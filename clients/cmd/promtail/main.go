...
func main() {
	...
	logutil.InitLogger(&config.ServerConfig.Config, prometheus.DefaultRegisterer)
	...
	if err != nil {
		level.Error(logutil.Logger).Log("msg", "error creating promtail", "error", err)
		os.Exit(1)
	}

	level.Info(logutil.Logger).Log("msg", "Starting Promtail", "version", version.Info())
	defer p.Shutdown()

	if err := p.Run(); err != nil {
		level.Error(logutil.Logger).Log("msg", "error starting promtail", "error", err)
		os.Exit(1)
	}
}
