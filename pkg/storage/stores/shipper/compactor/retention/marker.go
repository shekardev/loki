...
func (m *markerStorageWriter) createFile() error {
	...
	level.Info(logutil.Logger).Log("msg", "mark file created", "file", fileName)
...
func (r *markerProcessor) Start(deleteFunc func(ctx context.Context, chunkId []byte) error) {
	level.Info(logutil.Logger).Log("msg", "mark processor started", "workers", r.maxParallelism, "delay", r.minAgeFile)
	...
	go func() {
		...
		for ; true; tick() {
			...
			if err != nil {
				level.Error(logutil.Logger).Log("msg", "failed to list marks path", "path", r.folder, "err", err)
				continue
			}
			if len(paths) == 0 {
				level.Info(logutil.Logger).Log("msg", "no marks file found")
				r.sweeperMetrics.markerFileCurrentTime.Set(0)
				continue
			}
			for i, path := range paths {
				level.Debug(logutil.Logger).Log("msg", "processing mark file", "path", path)
				if r.ctx.Err() != nil {
					return
				}
				r.sweeperMetrics.markerFileCurrentTime.Set(float64(times[i].UnixNano()) / 1e9)
				if err := r.processPath(path, deleteFunc); err != nil {
					level.Warn(logutil.Logger).Log("msg", "failed to process marks", "path", path, "err", err)
					continue
				}
				// delete if empty.
				if err := r.deleteEmptyMarks(path); err != nil {
					level.Warn(logutil.Logger).Log("msg", "failed to delete marks", "path", path, "err", err)
				}
			}
...
func (r *markerProcessor) processPath(path string, deleteFunc func(ctx context.Context, chunkId []byte) error) error {
	...
	defer func() {
		if err := os.Remove(viewFile.Name()); err != nil {
			level.Warn(logutil.Logger).Log("msg", "failed to delete view file", "file", viewFile.Name(), "err", err)
		}
	}()
	...
	defer func() {
		if err := dbView.Close(); err != nil {
			level.Warn(logutil.Logger).Log("msg", "failed to close db view", "err", err)
		}
	}()
	...
	defer func() {
		...
		if err := dbUpdate.Close(); err != nil {
			level.Warn(logutil.Logger).Log("msg", "failed to close db", "err", err)
		}
	}()
		...
		go func() {
			...
			for key := range queue {
				if err := processKey(r.ctx, key, dbUpdate, deleteFunc); err != nil {
					level.Warn(logutil.Logger).Log("msg", "failed to delete key", "key", key.key.String(), "value", key.value.String(), "err", err)
				}
				putKeyBuffer(key)
			}
		}()
...
func (r *markerProcessor) availablePath() ([]string, []time.Time, error) {
	...
	if err := filepath.WalkDir(r.folder, func(path string, d fs.DirEntry, err error) error {
		...
		if err != nil {
			level.Warn(logutil.Logger).Log("msg", "wrong file name", "path", path, "base", base, "err", err)
			return nil
		}
